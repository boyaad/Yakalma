...
function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Catalogue", path: "/plats", icon: UtensilsCrossed },
  ];
    ...
    <nav className="w-full bg-white sticky top-0 z-40 shadow-sm">
      <div className="w-full px-6 lg:px-10">
        <div className="flex h-18 items-center justify-between gap-7">
          <Link to="/" className="flex items-center shrink-0" aria-label="Yakalma">
            <img
              src={logo}
              alt="Yakalma"
...
function NavbarLink({ link, isActive, onClick, mobile = false }) {
  ...
  return (
    <Link
      ...
      className={`inline-flex items-center gap-3 rounded-xl font-poppins font-semibold transition-all ${
        mobile ? "h-12 px-4 text-base" : "h-12 px-6 text-xl"
      } ${
        isActive
